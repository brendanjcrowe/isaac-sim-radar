...
echo "  1. The scene loads automatically on container start (run_headless.py is the default entrypoint)."
echo "     To run a timed session manually:"
echo "       docker exec isaac-sim /isaac-sim/python.sh /app/isaac_sim_scripts/run_headless.py --duration 120"
echo "  2. Check ROS2 topics appear:"
echo "       docker exec ros2-bridge bash -c 'source /opt/ros/humble/setup.bash && ros2 topic list'"
echo "  3. Start SLAM mapping:"
echo "       docker exec ros2-bridge ros2 launch /app/launch/slam.launch.py"
echo "  4. Start autonomous navigation (after SLAM is running):"
echo "       docker exec ros2-bridge ros2 launch /app/launch/nav2.launch.py"
echo "  5. Record a bag:"
