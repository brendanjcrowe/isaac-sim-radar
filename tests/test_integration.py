...
def _make_packet(detections: list[dict], timestamp_ns: int = 123456789) -> bytes:
    """Build a valid RDR2 binary packet from a list of detection dicts.

    Each dict must have keys: x, y, z, velocity, rcs, snr.
    """
    num = len(detections)
    header = struct.pack(HEADER_FORMAT, HEADER_MAGIC, num, timestamp_ns)
    body = b""
    for d in detections:
        body += struct.pack(
            "<ffffff",
            d["x"], d["y"], d["z"],
            d["velocity"], d["rcs"], d["snr"],
        )
    return header + body
...
class TestFullBytePipeline(unittest.TestCase):
    ...
    def setUp(self):
        self.detections = [
            {"x": 1.0,  "y": 2.0, "z": 0.5, "velocity": -3.5, "rcs": -10.0, "snr": 15.0},
            {"x": -4.0, "y": 0.0, "z": 1.2, "velocity":  0.0, "rcs":  -5.0, "snr": 20.0},
            {"x": 0.0,  "y": 7.5, "z": 0.0, "velocity":  2.1, "rcs":   3.0, "snr":  8.0},
        ]
    ...
    def test_parse_recovers_timestamp(self):
        frame = parse_generic_model_output(self.packet)
...
class TestPointCloud2DataLayout(unittest.TestCase):
    ...
    def setUp(self):
        det = {"x": 10.0, "y": 20.0, "z": 30.0, "velocity": 1.5, "rcs": -8.0, "snr": 12.0}
